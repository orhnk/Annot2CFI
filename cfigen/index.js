...
(() => {
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.readFile(filePath, "utf8", (err, data) => {
        try {
          if (err) {
            throw new Error(
              `Error reading container.xml file ${filePath}: ${err}`,
            );
          }
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(packageFilePath, "utf8", (err, data) => {
        try {
          if (err) {
            throw new Error(
              `Error reading package file ${packageFilePath}: ${err}`,
            );
          }

          const doc = domParser.parseFromString(data);
          const itemrefs = xpathUtils.nsXPath(
            xpathUtils.NS_PACKAGE_DOC,
            "//main:spine//main:itemref",
            doc,
          );
          ...
          resolve(
            itemrefs.map((itemref) => {
              ...
              return {
                spineCfi: cfiGenerator.createCFIElementSteps(
                  itemref,
                  "package",
                  null,
                  null,
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(spineFilePath, "utf8", (err, data) => {
        try {
          if (err) {
            throw new Error(
              `Error reading spine file ${spineFilePath}: ${err}`,
            );
          }

          const doc = domParser.parseFromString(data);

          const textNodes = xpathUtils.htmlXPath(
            "/main:html/main:body//text()[normalize-space()]",
            doc,
          );

          if (DEBUG) {
            console.log(
              `spine file ${spineFilePath}, length: ${data.length}, number of text nodes: ${textNodes.length}`,
            );
          }

          resolve(
            textNodes.map((node) => {
              const cfi = cfiGenerator.generateCharacterOffsetCFIComponent(
                node,
                null,
                null,
                null,
  ...
  EpubCfiGenerator.prototype.parse = (inputPath) =>
    new Promise((resolve, reject) => {
      ...
      .then(getPackageFilePath)
      .then((packageFilePath) => getSpinesInfo(packageFilePath))
      .then((spinesInfo) =>
        Promise.all(
          spinesInfo.map((spineInfo) =>
            getSpineNodesCfi(spineInfo.path, spineInfo.spineCfi).then(
              (nodes) => {
